...
const agentInfo = [
    ...
    { name: '通用医疗咨询', icon: '🩺', desc: '全科医学综合咨询与健康指导', color: 'cyan', colorHex: '#0891b2' },
];
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch('/api/analysis/start', {
                ...
                body: JSON.stringify({ patientName: patient.name || '未命名', age: patient.age, gender: patient.gender, fileText: text, analysisMode, location }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || '启动分析失败');
            if (data.taskId) { connectWS(data.taskId, data.recordId); }
            else { throw new Error(data.error || '启动分析失败'); }
        } catch (err) {
            setStatus('failed');
            addToast(err instanceof Error ? err.message : '网络请求失败', 'error');
        }
    };
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="glass-card p-5">
                        ...
                        <p className="text-xs text-gray-400 mt-2">{analysisMode === 'full' ? '5个智能体完整协作，提供详细报告' : '快速评估核心风险，给出关键建议'}</p>
                    </div>
