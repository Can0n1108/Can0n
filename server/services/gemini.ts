...
export async function runMultiAgentAnalysis(inputText: string): Promise<AgentResult[]> {
    if (!isValidKey) {
...
function generateMockAnalysis(inputText: string): AgentResult[] {
    ...
    if (!p.hasDigestive && !p.hasHeart && !p.hasDiabetes && !p.hasFever) {
        symptoms.push('需进一步检查明确');
        conditions.push({ name: '待排查疾病', probability: '中', reason: '症状描述不够完整，建议完善检查' });
    }
    ...
    if (abnormalFindings.length === 0) {
        abnormalFindings.push({ item: '待检测指标', value: '未提供', reference: '-', significance: '建议完善血常规、生化全项等检查' });
        keyObs.push('未提供具体检验数值，建议完善检查');
    }
    ...
    if (riskFactors.length === 0) {
        if (p.hasHeadache) {
            ...
            riskFactors.push({ factor: '信息不足', severity: '低', description: '当前信息不足以进行完整风险评估' });
            complications.push('需进一步评估');
        }
    }
    ...
    if (treatmentPlan.length === 0) {
        ...
        if (p.hasHeadache) {
            ...
            treatmentPlan.push({ step: '完善检查', detail: '建议完善血常规、生化全项、影像学等检查以明确诊断', priority: '重要' });
            treatmentPlan.push({ step: '定期随访', detail: '根据检查结果制定后续方案', priority: '常规' });
            medication.push({ drug: '根据诊断结果选择', note: '请在医生指导下用药' });
            followUp.push('1-2周后复查相关指标');
            lifestyle.push('保持充足休息', '清淡饮食', '多饮水', '避免劳累');
        }
    }

    const hasRealSymptoms = conditions.length > 0 && conditions[0].name !== '待排查疾病' && conditions[0].name !== '信息不足';
    const hasRealLabData = abnormalFindings.length > 0 && abnormalFindings[0].item !== '待检测指标';
    const hasRealRisk = riskFactors.length > 0 && riskFactors[0].factor !== '信息不足';
    const hasRealTreatment = treatmentPlan.length > 0 && treatmentPlan[0].step !== '完善检查';
